...
class Iamport
{
    ...
    public function findByImpUID($imp_uid)
    {
        try {
            ...
            return new IamportResult(false, null, ['code'=>$e->getCode(), 'message'=>$e->getMessage()]);
    ...
    public function cancel($data)
    {
        try {
            ...
            if ($data['imp_uid']) {
                ...
                return new IamportResult(false, null, ['code'=>'', 'message'=>'취소하실 imp_uid 또는 merchant_uid 중 하나를 지정하셔야 합니다.']);
            }
            $response = $this->postResponse(
                self::CANCEL_PAYMENT_URL,
                $cancel_data,
                [self::TOKEN_HEADER.': '.$access_token]
                );
            $payment_data = new IamportPayment($response);
    ...
    {
        $access_token = $this->getAccessCode();
        $headers      = [self::TOKEN_HEADER.': '.$access_token, 'Content-Type: application/json'];
        $ch           = curl_init();
    ...
    private function postResponse($request_url, $post_data=[], $headers=[])
    {
        $post_data_str  = json_encode($post_data);
        $default_header = ['Content-Type: application/json', 'Content-Length: '.strlen($post_data_str)];
    ...
    private function deleteResponse($request_url, $headers=[])
    {
